...
transaction(listingResourceID: UInt64, storefrontAddress: Address, commissionRecipient: Address?) {
    ...
    let CardCollectionPublic: &BBxBarbieCard.Collection{NonFungibleToken.Receiver}
    ...
    prepare(acct: AuthAccount) {
        ...
        let mainFlowVault = acct.borrow<&FiatToken.Vault>(from: FiatToken.VaultStoragePath)
            ?? panic("Cannot borrow FiatToken vault from acct storage")
        self.paymentVault <- mainFlowVault.withdraw(amount: price)

        // Access the buyer's NFT collection to store the purchased NFT.
        self.CardCollectionPublic = acct.borrow<&BBxBarbieCard.Collection{NonFungibleToken.Receiver}>(
            from: BBxBarbieCard.CollectionStoragePath
        ...
        if commissionRecipient != nil && commissionAmount != 0.0 {
            ...
            let _commissionRecipientCap = getAccount(commissionRecipient!).getCapability<&{FungibleToken.Receiver}>(/public/GenericFTReceiver)
    ...
    execute {
        ...
        self.CardCollectionPublic.deposit(token: <-item)
    }
}