...
access(all) contract HWGaragePM {
    /* 
    *   Events
    *
    ...
    */
    access(all) event AdminMintHWGarageCard(id: UInt64)
    // emmited when the metadata for an HWGarageCard collection is updated
    access(all) event UpdateHWGarageCardCollectionMetadata()
    // emmited when an edition within HWGarageCard has had it's metdata updated
    access(all) event UpdateTokenEditionMetadata(id: UInt64, metadata: {String: String}, packHash: String, address: Address)
    ...
    */
    access(all) event AdminMintPack(id: UInt64, packHash: String)
    // emitted when someone redeems a HWGaragePack for Cards
    access(all) event RedeemPack(id: UInt64, packID: UInt64, packEditionID: UInt64, address: Address, packHash: String)
    // emitted when Pack has metadata updated
    access(all) event UpdatePackCollectionMetadata()
    // emitted when PackEdition updates metadata
    access(all) event UpdatePackEditionMetadata(id: UInt64, metadata: {String: String})
    // emitted when any information about redemption has been modified
    access(all) event UpdateHWGaragePackRedeemInfo(redeemStartTime: UFix64)
    // emitted when a user submits a packHash to claim a pack
    access(all) event PackClaimBegin(address: Address, packHash: String)
    // emitted when a user succesfully claims a pack
    access(all) event PackClaimSuccess(address: Address, packHash: String, packID: UInt64)
    ...
    *   emitted when an admin has initiates an airdrop to a wallet
     */
    // V2 is depracated
    access(all) event AirdropRedeemableV2(id:UInt64, address: Address)
    ...
    */
    ...
    access(all) var HWGarageCardsTotalSupply: UInt64
    /* 
    *   HWGaragePack
    */
    ...
    access(all) var packTotalSupply: UInt64
    access(all) var packRedeemStartTime: UFix64

    access(all) entitlement UpdateMetadata
    access(all) entitlement MintResource
    access(all) entitlement UpdateStartTime

    /* 
    *   Manager resource for all NFTs
    */
    access(all) resource Manager { // FlowToken follows similar pattern
        /* 
        *   HWGarageCard
        */
        access(all) fun updateHWGarageCardEditionMetadata(editionNumber: UInt64, metadata: {String: String}, packHash: String, address: Address) {
        ...
        access(all) fun updateHWGarageCardCollectionMetadata(metadata: {String: String}) {
            HWGarageCard.setCollectionMetadata(metadata: metadata)
            emit UpdateHWGarageCardCollectionMetadata()
        }

        access(all) fun mintHWGarageCardAtEdition(edition: UInt64, packID: UInt64): @{NonFungibleToken.NFT} {
            emit AdminMintHWGarageCard(id: edition)
            return <-HWGaragePM.mintHWGarageCard(edition: edition, packID: packID)
        }

        access(all) fun mintSequentialHWGarageCard(packID: UInt64): @{NonFungibleToken.NFT} {
        ...
        *   HWGaragePack
        */
        access(all) fun updatePackEditionMetadata(editionNumber: UInt64, metadata: {String: String}) {
        ...
        access(all) fun updatePackCollectionMetadata(metadata: {String: String}) {
            HWGaragePack.setCollectionMetadata(metadata: metadata)
            emit UpdatePackCollectionMetadata()
        }

        access(all) fun mintPackAtEdition(edition: UInt64, packID: UInt64, packEditionID: UInt64, address: Address, packHash: String): @{NonFungibleToken.NFT} {
            emit AdminMintHWGarageCard(id: edition)
        ...
        access(all) fun mintSequentialHWGaragePack(packID: UInt64, address: Address, packHash: String): @{NonFungibleToken.NFT} {
        ...
        access(all) fun updateHWGaragePackRedeemStartTime(_ redeemStartTime: UFix64) {
    ...
    *   Mint a HWGarageCard
    */
    access(self) fun mintHWGarageCard(edition: UInt64, packID: UInt64): @{NonFungibleToken.NFT} {
        pre {
        ...
        let hwGarageCard: @HWGarageCard.NFT <- HWGarageCard.mint(nftID: edition, packID: packID)
        return <-hwGarageCard
    }
    ...
    access(self) fun mintSequentialHWGarageCard(packID: UInt64): @{NonFungibleToken.NFT} {
        ...
        let hwGarageCard: @{NonFungibleToken.NFT} <- self.mintHWGarageCard(edition: currentEditionNumber, packID: packID)
        return <- hwGarageCard
    }
    ...
    *   Mint a HWGaragePack
    */
    access(self) fun mintPackAtEdition(edition: UInt64, packID: UInt64, packEditionID: UInt64, packHash: String): @{NonFungibleToken.NFT} {
        pre {
            edition >= 1: "Requested edition is outside the realm of space and time, you just lost the game."
       }

        let pack: @HWGaragePack.NFT <- HWGaragePack.mint(nftID: edition, packID: packID, packEditionID: packEditionID)
        return <-pack
    }
    ...
    access(self) fun mintSequentialPack(packID: UInt64, packHash: String): @{NonFungibleToken.NFT} {
        ...
        let newToken: @{NonFungibleToken.NFT} <- self.mintPackAtEdition(edition: UInt64(currentPackEditionNumber), packID: packID, packEditionID: UInt64(currentPackEditionNumber), packHash: packHash)
        return <-newToken
    }
    ...
    */
    access(self) fun mintSequentialAirdrop(airdropID: UInt64): @{NonFungibleToken.NFT} {
    ...
    */
    ...
    access(all) fun publicRedeemPack(pack: @{NonFungibleToken.NFT}, address: Address, packHash: String) {
        pre {
