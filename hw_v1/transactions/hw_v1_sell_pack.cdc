...
transaction(
    ...
    let HWGaragePackProvider: Capability<auth(NonFungibleToken.Withdraw) &{NonFungibleToken.Collection}>
    ...
    prepare(acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue) &Account) {
        ...
        let collectionData: MetadataViews.NFTCollectionData = HWGaragePack.resolveContractView(resourceType: nil, viewType: Type<MetadataViews.NFTCollectionData>()) as! MetadataViews.NFTCollectionData?
        ...
        self.HWGaragePackProvider = acct.capabilities.storage.issue<auth(NonFungibleToken.Withdraw) &{NonFungibleToken.Collection}>(
                collectionData.storagePath
            )

        assert(self.HWGaragePackProvider.check(), message: "Missing or mis-typed ExampleNFT provider")

        let collection: &{NonFungibleToken.Collection} = acct.capabilities.borrow<&{NonFungibleToken.Collection}>(
                collectionData.publicPath
            ) ?? panic("Could not borrow a reference to the signer's collection")

        var totalRoyaltyCut: UFix64 = 0.0
        let effectiveSaleItemPrice: UFix64 = saleItemPrice - commissionAmount
        let nft: &{NonFungibleToken.NFT} = collection.borrowNFT(saleItemID)!
        // Check whether the NFT implements the MetadataResolver or not.
        if nft.getViews().contains(Type<MetadataViews.Royalties>()) {
            let royaltiesRef: AnyStruct = nft.resolveView(Type<MetadataViews.Royalties>())?? panic("Unable to retrieve the royalties")
            let royalties: [MetadataViews.Royalty] = (royaltiesRef as! MetadataViews.Royalties).getRoyalties()
    ...
    execute {
        self.storefront.createListing(
            nftProviderCapability: self.HWGaragePackProvider,
            nftType: Type<@HWGaragePack.NFT>(),
            nftID: saleItemID,
